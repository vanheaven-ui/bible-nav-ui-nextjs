...
const VerseItem: React.FC<VerseItemProps> = memo(
  ({ verse, bookName, chapterNumber, isFav, userNote, toggleFavorite }) => {
    ...
    // Helper function to get plain text from the Slate JSON content
...
const ChapterVersesPage: React.FC = () => {
  ...
  const toggleFavorite = useCallback(
      ...
      try {
        if (existing) {
          ...
          setFavorites((prevFavs) =>
            prevFavs.filter((f) => f.id !== existing.id)
          );
        } else {
    ...
    [favorites, token, bookName, chapterNumber]
  );
  ...
  useEffect(() => {
    const loadData = async () => {
      const decodedBookName = bookName
        ? decodeURIComponent(bookName as string)
        : null;
      const currentChapterNumber = chapterNumber ? Number(chapterNumber) : null;

      if (!decodedBookName || !currentChapterNumber) {
      ...
      try {
        const [allBooks, bibleChapter] = await Promise.all([
          fetchBibleBooks(),
          fetchBibleChapter(decodedBookName, currentChapterNumber),
        ]);
          ...
          <div className="space-y-4">
            {chapter?.verses.map((v) => {
              ...
              const userNote = userNotes.find(
                (n) =>
                  n.book === decodeURIComponent(bookName as string) &&
                  n.chapter === Number(chapterNumber) &&
